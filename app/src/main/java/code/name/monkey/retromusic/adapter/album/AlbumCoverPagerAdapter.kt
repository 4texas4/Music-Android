...
class AlbumCoverPagerAdapter(
    ...
    class AlbumCoverFragment : Fragment() {
        ...
        private var lyricsCacheFile: File? = null
        private lateinit var lyricsWriteLauncher: ActivityResultLauncher<IntentSenderRequest>
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            lyricsWriteLauncher = registerForActivityResult(
                ActivityResultContracts.StartIntentSenderForResult()
            ) { result ->
                if (result.resultCode == android.app.Activity.RESULT_OK) {
                    lyricsCacheFile?.let { cache ->
                        requireContext().contentResolver.openOutputStream(song.uri)?.use { out ->
        ...
        private fun fetchAndInjectLyrics() {
            ...
            lifecycleScope.launch {
                try {
                    ...
                    val toSave = result?.syncedLyrics ?: result?.plainLyrics
                    val displayText = result?.syncedLyrics ?: result?.plainLyrics
                    ...
                    if (toSave.isNullOrEmpty()) {
                        ...
                        // Show preview
                        MaterialAlertDialogBuilder(requireContext())
                            .setTitle("Lyrics Preview")
                            .setMessage(displayText)
                            .setPositiveButton("Add to Song") { _, _ ->
                                lifecycleScope.launch {
                                    val fieldMap = EnumMap<FieldKey, String>(FieldKey::class.java)
                                    fieldMap[FieldKey.LYRICS] = toSave
                                    if (VersionUtils.hasR()) {
                                        val cacheFiles = withContext(Dispatchers.IO) {
                                            TagWriter.writeTagsToFilesR(
                                                requireContext(),
                                                AudioTagInfo(listOf(currentSong.data), fieldMap, null)
                                            )
                                        }
                                        if (cacheFiles.isNotEmpty()) {
                                            lyricsCacheFile = cacheFiles[0]
